...
local COLLECT_RANGE = 100
local autoCollectRunning = false
local requestedDrops = {}

local function getAutoCollectEvent()
    local remotes = ReplicatedStorage:FindFirstChild("Remotes")
    if not remotes then
        return nil
    end

    return remotes:FindFirstChild("AutoCollectRequest")
end
...
local function collectAllDrops()
    ...
    local autoCollectEvent = getAutoCollectEvent()

    if not autoCollectEvent then
        pcall(function()
            StarterGui:SetCore("SendNotification", {
                Title = "TAKE ALL",
                Text = "AutoCollectRequest tidak ditemukan.",
                Duration = 3
            })
        end)
        return
    ...
    if not dropFolder or not hrp then
    ...
    for _, drop in ipairs(dropFolder:GetChildren()) do
        if drop.Parent and not requestedDrops[drop] then
            ...
            if position and (hrp.Position - position).Magnitude <= COLLECT_RANGE then
...
local function setupDropTracking()
    local dropFolder = Workspace:FindFirstChild("Drops")
...
local function createMainGui()
    ...
    crystalButton.MouseButton1Click:Connect(function()
        ...
        -- Instant reset: no 6-second client-side delay.
        resetCooldown = false
